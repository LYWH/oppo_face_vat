...
def main():
    ...
    #
    #
    # train_loader = torch.utils.data.DataLoader(data_iterators,
    #                                           batch_size=4,
    #                                           shuffle=True,
    #                                           num_workers=4)

    cifar = datasets.CIFAR10('D:/cifar10', True, transform=transforms.Compose([
        transforms.Resize([32, 32]),
        transforms.ToTensor()
    ]), download=False)
    cifar_train = DataLoader(cifar, batch_size=4, shuffle=True)
